...
fn test_transaction_info_and_proof_1() -> Result<()> {
    ...
    let dag_fork_height = block_chain.dag_fork_height()?.unwrap();
    assert_eq!(dag_fork_height, 0);

    let _current_header = block_chain.current_header();
    let miner_account = AccountInfo::random();
    let mut seq_num = 0;
    // generate DAG_FORK_HEIGHT + 5 blocks
    let block_count = dag_fork_height
        .checked_add(5)
    ...
    // fork from DAG_FORK_HEIGHT + 3 block
    let fork_count = dag_fork_height
        .checked_add(3)
